...
- NFR-P1: 매물 검색 응답성
- NFR-P2: 경매/입찰 조회 효율
- NFR-P3: 중개사별 입찰 이력 조회 효율
- NFR-P4: 검색 결과 상한
- NFR-U1: 콘솔 입력 오류 복구성
- NFR-U2: 입력 오류 메시지 명확성
- NFR-O1: 처리 결과 관찰가능성
- NFR-R1: 조회 결과 방어적 복사
- NFR-M1: 기존 L3/L4 구조 유지
- NFR-T1: 동일 데이터셋 기반 테스트 가능성
...
대표 테스트는 6개만 실행한다. 모든 NFR을 억지로 테스트하기보다, 발표에서 변화가 잘 보이는 성능/사용성/관찰가능성 항목을 선택했다.
...
| 테스트 | L3+L4 | SA | 요약 |
|---|---:|---:|---|
| 매물 지역 검색 | 92.543 ms | 4.860 ms | 약 19.0배 차이 |
| 경매별 입찰 조회 | 62.675 ms | 0.042 ms | 약 1506.6배 차이 |
| 중개사별 입찰 이력 조회 | 73.229 ms | 0.055 ms | 약 1343.7배 차이 |
| 검색 결과 상한 | 6.936 ms | 1.140 ms | 약 6.1배 차이 |
| 처리 로그 | 결과만 반환 | `OperationLog` 1건 생성 | 관찰가능성 차이 확인 |
